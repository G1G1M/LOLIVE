...
extension FavoriteTeamWidgetView {
    ...
    var mediumView: some View {
        GeometryReader { geo in
            ...
            // 실제 계산해서 맞춘 값 — Medium 캔버스 최소 높이(가장 작은 기기 기준 약 155pt)에서
            // 패딩 20(위아래 40) + 캐러셀 줄(36, 여러 팀일 때만) + 여백을 다 빼고 남는 공간에
            // 로고(44)+텍스트가 겨우 들어가는 수준이라, 간격을 6pt 선에서 확정함(그 이상은
            // 작은 기기에서 실제로 잘림 — 실측 확인).
            VStack(spacing: 0) {
