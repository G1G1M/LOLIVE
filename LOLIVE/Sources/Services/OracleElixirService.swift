...
struct OracleElixirService: Sendable {
    ...
    private func oeGameId(riotMatchId: String, gameNumber: Int) async -> String? {
        let cacheKey = "oe_single_match_\(riotMatchId)"
        let rows: [SingleMatchRow]
        if let cached: [SingleMatchRow] = AppDiskCache.get(key: cacheKey, maxAge: 30 * 24 * 3600) {
            rows = cached
        } else {
            guard let url = URL(string: "\(apiBase)/matches/singleMatch/\(riotMatchId)") else { return nil }
            var request = URLRequest(url: url)
            request.setValue(apiKey, forHTTPHeaderField: "x-api-key")
            guard let (data, response) = try? await Self.session.data(for: request),
                  (response as? HTTPURLResponse)?.statusCode == 200,
                  let decoded = try? JSONDecoder().decode([SingleMatchRow].self, from: data)
            else { return nil }
            rows = decoded
            AppDiskCache.set(key: cacheKey, value: rows)
        }
    ...
    /// Riot이 밴 데이터를 안 주는 경기의 폴백. `MatchDetailViewModel`이 기존에 Leaguepedia로만
    /// 채우던 걸 OE `/drafts/{gameId}`로 먼저 시도하도록 추가 — 실패하면 호출부가 Leaguepedia로
    /// 폴백한다.
    func fetchDraftBans(riotMatchId: String, gameNumber: Int) async -> (blue: [String], red: [String])? {
        guard let gameId = await oeGameId(riotMatchId: riotMatchId, gameNumber: gameNumber) else { return nil }

        let cacheKey = "oe_draft_\(gameId)"
        let draft: DraftRow
        if let cached: DraftRow = AppDiskCache.get(key: cacheKey, maxAge: 30 * 24 * 3600) {
            draft = cached
        } else {
            guard let encoded = gameId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed),
                  let url = URL(string: "\(apiBase)/drafts/\(encoded)")
            else { return nil }
            var request = URLRequest(url: url)
            request.setValue(apiKey, forHTTPHeaderField: "x-api-key")
            guard let (data, response) = try? await Self.session.data(for: request),
                  (response as? HTTPURLResponse)?.statusCode == 200,
                  let decoded = try? JSONDecoder().decode(DraftRow.self, from: data)
            else { return nil }
            draft = decoded
            AppDiskCache.set(key: cacheKey, value: draft)
        }
        return Self.splitBansBySide(draft.bansInOrder, firstPick: draft.firstPick)
    }

    // MARK: - stuck 라이브 경기 보정
    ...
    func reconcileStuckLiveMatch(_ match: Match) async -> Match? {
        guard let url = URL(string: "\(apiBase)/matches/singleMatch/\(match.id)") else { return nil }
        var request = URLRequest(url: url)
        request.setValue(apiKey, forHTTPHeaderField: "x-api-key")
        guard let (data, response) = try? await Self.session.data(for: request),
              (response as? HTTPURLResponse)?.statusCode == 200,
              let rows = try? JSONDecoder().decode([SingleMatchStateRow].self, from: data),
              let row = rows.first,
