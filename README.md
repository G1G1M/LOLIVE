...
- **네이티브 검색창 확장 애니메이션**: 직접 만든 텍스트필드 대신 SwiftUI `.searchable(text:)`를 사용 —
  Search 탭을 누르면 원형 버튼이 시스템이 그려주는 검색창으로 늘어나며 자동으로 포커스까지 잡힘
  (애플 표준 Search Role 동작). 이 효과가 나오려면 Search 탭의 콘텐츠가 자체 `NavigationStack`을
  가져야 해서 `SearchView`에 추가함 — 이전엔 tab 6개라 "더보기" 안에 있었던 탓에 자체
  `NavigationStack`을 일부러 안 뒀었는데, Search가 5개 탭 중 하나로 직접 노출되면서 필요해짐
- 리그 / 팀 / 선수 통합 검색
