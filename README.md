...
- **탭 선택 즉시 키보드 자동 표시**: `.searchable`만으로는 검색창이 펼쳐지기만 하고 키보드는 안 뜨길래,
  `isPresented` 바인딩이 true가 되는 시점(탭 선택)에 iOS 18+ 전용 `.searchFocused`로 직접 포커스를
  줘서 탭 누르자마자 바로 타이핑 가능하게 함. iOS 17 폴백 탭바에선 이 모디파이어 자체가 없어서
  기존처럼 수동 탭 후 입력
- 리그 / 팀 / 선수 통합 검색
