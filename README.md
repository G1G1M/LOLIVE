...
- **LCK 등 "스플릿 넘어 누적되는" 순위표 대응**: LCK는 2026년부터 스플릿마다 리셋되지 않고
  레전드/라이즈 그룹 단위로 시즌 전체 누적 성적을 보여준다(Leaguepedia에 공개된 실제 표 기준 —
  한 스플릿 경기 수를 훨씬 넘는 기록, 예: HLE 15승4패). 그런데 Riot API의 `getStandings`는 스플릿마다
  리셋된 값만 주고(같은 시점 실제로는 2승0패), `fetchSchedule`도 최근 몇 페이지로 윈도우가 좁아 시즌
  전체를 못 담는다 — 그래서 `fetchAllSchedule`(과거 페이지 전체 순회, 이미 있던 함수)로 시즌 전체
  일정을 가져와 `seasonStartDate` 이후 완료 경기를 전부 합산한다. 그룹(레전드/라이즈) 배정은 Riot의
  최신 스플릿 순위 응답을 그대로 따르고, 승패 숫자만 누적으로 재계산.
  **`seasonStartDate` 기준을 두 번 고침**: (1) 처음엔 "현재 토너먼트 기간 밖 경기는 제외"하는
  방향으로 고쳤다가, 실제 Leaguepedia 표와 대조해보고 방향이 반대(스플릿 리셋이 아니라 누적)라는
  걸 확인해 뒤집음. (2) 그다음 "같은 해 첫 스플릿부터 전부 누적"으로 고쳤더니 이번엔 실측보다 경기
  수가 훨씬 많이 나옴 — 확인해보니 스플릿마다 대회 형식 자체가 다름(2026 LCK 기준 Split 1은
  "알파/오메가 그룹", Split 2는 "그룹 없는 단일 정규리그", Split 3이 지금의 "레전드/라이즈 그룹").
  실측과 대조해 "바로 직전 스플릿부터 현재 스플릿까지"가 맞다는 결론으로 최종 수정
