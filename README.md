...
- Live Activity: 잠금화면 실시간 스코어 (`[로고] 팀명 스코어–스코어 팀명 [로고]`) + Dynamic Island
  ...
  - **Riot 라이브 피드가 멈춘 경우 대응**: `esports-api.../getLive`가 `inProgress`로 계속 보고해도 실제로는
    ...
    경기에서 40분 넘게 프레임이 안 들어온 사례). 스코어/세트 번호는 원래 세트가 끝나야만 바뀌는 값이라
    "안 바뀐다"는 걸로 멈춤을 판단할 수 없어서, 대신 인게임 피드의 최신 프레임 시각을 5분 쿨다운으로
    직접 확인 → 5분 이상 갱신이 없으면 Leaguepedia로 대조해서 이미 끝난 경기면 로컬에서 완료 처리
    (결과 알림·Live Activity 종료·목록 이동까지 자연스럽게 이어짐)
