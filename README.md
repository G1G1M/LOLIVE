...
- **LCK 등 "스플릿 넘어 누적되는" 순위표 대응**: LCK는 2026년부터 스플릿마다 리셋되지 않고
  ...
  일정을 가져와 `seasonStartDate`(같은 해에 시작한 정규 스플릿 중 가장 이른 시작일, 플레이오프/컵
  제외) 이후 완료 경기를 전부 합산한다. 그룹(레전드/라이즈) 배정은 Riot의 최신 스플릿 순위 응답을
  그대로 따르고, 승패 숫자만 시즌 누적으로 재계산. **중간 시행착오**: 처음엔 반대로 "현재 토너먼트
  기간 밖 경기는 제외"하는 방향으로 고쳤다가, 실제 Leaguepedia 표와 대조해보고 나서야 방향이 반대
  (스플릿 리셋이 아니라 시즌 누적이 맞음)라는 걸 확인해 다시 고침
