...
- Live Activity: 잠금화면 실시간 스코어 (`[로고] 팀명 스코어–스코어 팀명 [로고]`) + Dynamic Island
  ...
  - **Riot 라이브 피드가 멈춘 경우 대응**: `esports-api.../getLive`가 `inProgress`로 계속 보고해도 실제로는
    ...
    "멈춤" 신호로 사용한다. 멈춘 것으로 확인되면 2단계로 Leaguepedia와 대조:
    1) `ScoreboardGames`(세트 단위) — 시리즈가 안 끝나도 방금 끝난 세트만 반영, 2) `MatchSchedule`(시리즈
    단위) — 시리즈 자체가 끝났으면 완료 처리(결과 알림·Live Activity 종료·목록 이동). 둘 다 없으면 점수를
    지어내지 않고 5분마다 재시도만 한다.
