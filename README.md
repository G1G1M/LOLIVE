...
- **예정 시각 경과 표시**: 예정 시각이 지났는데도 Riot이 상태를 `unstarted`로 안 바꿔주는 경우(중계 지연 등),
  확정 LIVE와는 다른 주황색 "진행 중일 수 있음" 배지로 구분 표시 — 예정 시각이 그냥 지나간 것처럼 보이지 않게 함
  (Today 카드·경기 상세 화면 모두 적용. Riot 공식 API 자체가 아직 상태를 안 준 경우라 진짜 스코어는 상태가 갱신돼야 나옴)
