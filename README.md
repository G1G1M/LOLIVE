...
| 오늘의 경기 | 리그 | 순위 | 선수 |
|:---:|:---:|:---:|:---:|
| <img src="docs/screenshots/today.png" width="200" alt="오늘의 경기 화면"> | <img src="docs/screenshots/leagues.png" width="200" alt="리그 목록 화면"> | <img src="docs/screenshots/standings.png" width="200" alt="순위 화면"> | <img src="docs/screenshots/players.png" width="200" alt="선수 목록 화면"> |
| 날짜별 경기를 리그로 묶어 보여주고, 진행 중인 경기는 LIVE 배지와 실시간 스코어로 표시합니다. | 추적 중인 45개 리그를 지역 순으로. 국제 대회(Worlds/MSI)는 연도별 기록 화면으로 따로 들어갑니다. | 완료된 경기 스코어로 승패·득실을 직접 재계산합니다 — 리그 API가 팀마다 반영 시점이 어긋나는 경우가 있어서입니다. | 1군 리그 선수 전체를 이름·팀·포지션으로 검색합니다. |

> 화면은 시스템 설정을 따라 라이트/다크 모드가 자동으로 바뀌며, 앱 설정에서 직접 고를 수도 있습니다.
...
폴링 결과가 직전과 같으면 `liveMatches`에 다시 대입하지 않습니다 — `@Observable` 프로퍼티는
같은 값이라도 대입하는 순간 이걸 읽는 뷰가 전부 무효화되기 때문입니다. 그래서 `LiveMatch`에는
조회 시각처럼 매번 달라지는 값을 넣지 않습니다.
...
(폴더 동기화 방식이라 파일만 놓으면 번들에 자동 포함됩니다).

- 추적 안 함, 추적 도메인 없음
- 서버로 나가는 데이터는 푸시 등록 한 곳뿐 — FCM 토큰(Device ID) + 즐겨찾기 팀 코드.
  계정이 없어 신원에 연결되지 않고 추적에도 쓰지 않습니다.
- 직접 쓰는 "필수 사유 API"는 UserDefaults 하나 — 앱 전용(`CA92.1`)과 App Group 공유(`1C8F.1`).
  위젯은 App Group만 씁니다.
- Firebase SDK(Core/Functions/Messaging)는 각자 매니페스트를 갖고 있어 따로 적지 않습니다.
